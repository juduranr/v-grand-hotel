        ...
        <div className="rooms-carousel__carousel">
          ...
          <div className="rooms-carousel__carousel-container">
            <div className="rooms-carousel__room-card">
              {/* Contenido estático de la habitación actual */}
              <div className="rooms-carousel__room-content">
                ...
                <h3 className="rooms-carousel__room-title">
                  {roomsData[currentIndex]?.title}
                </h3>
                <p className="rooms-carousel__room-capacity">
                  {roomsData[currentIndex]?.capacity} personas
                </p>
                {roomsData[currentIndex]?.description && (
                ...
                <div className="rooms-carousel__room-details">
                  <div className="rooms-carousel__all-icons">
                    {Array(roomsData[currentIndex]?.beds || 0)
                      .fill(<DoubleBed fill="#ffffff" strokeWidth={2} size={32} />)
                      .map((icon, i) => (
                        <span key={`beds-${i}`} className="rooms-carousel__room-icon">
                          {icon}
                        </span>
                      ))}
                    {Array(roomsData[currentIndex]?.bathrooms || 0)
                      .fill(<Tub fill="#ffffff" strokeWidth={2} size={32} />)
                      .map((icon, i) => (
                        <span key={`bathrooms-${i}`} className="rooms-carousel__room-icon">
                          {icon}
                        </span>
                      ))}
                    {roomsData[currentIndex]?.terrace && (
                    ...
                    {roomsData[currentIndex]?.breakfast && (
                  ...
                  <div className="rooms-carousel__all-texts">
                    <span>
                      {roomsData[currentIndex]?.capacity} persona
                      {roomsData[currentIndex] && roomsData[currentIndex].capacity > 1 ? "s" : ""}
                    </span>
                    <span>{roomsData[currentIndex]?.area}m²</span>
                    <span>
                      {roomsData[currentIndex]?.beds} cama{roomsData[currentIndex] && roomsData[currentIndex].beds > 1 ? "s" : ""}
                    </span>
                    <span>
                      {roomsData[currentIndex]?.bathrooms} baño{roomsData[currentIndex] && roomsData[currentIndex].bathrooms > 1 ? "s" : ""}
                    </span>
                    {roomsData[currentIndex]?.terrace && <span>terraza</span>}
                    {roomsData[currentIndex]?.breakfast && <span>desayuno incluido</span>}
                  </div>
                </div>
              </div>

              {/* Contenedor de imágenes con pista deslizante */}
              <div className="rooms-carousel__room-image-container">
                ...
                >
                  {/* Imagen duplicada del último para wrap hacia atrás */}
                  <div className="rooms-carousel__carousel-image-item">
                    <img
                  ...
                  {roomsData.map((room, index) => (
                    <div key={index} className="rooms-carousel__carousel-image-item">
                      <img
                  ...
                  {/* Imagen duplicada del primero para wrap hacia adelante */}
                  <div className="rooms-carousel__carousel-image-item">
                    <img
